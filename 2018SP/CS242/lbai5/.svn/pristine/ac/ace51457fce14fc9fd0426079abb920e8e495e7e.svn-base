...
public class Network {
	...
	public boolean requestBroadcast(Writer report) {
		...
		do {
			try {
				...
				String message1 = "' accepts broadcast packet.\n";
				String message2 = "' passes packet on.\n";
				node.printRequestMessage(report, this, message1);
				node.printRequestMessage(report, this, message2);
				report.flush();
			} catch (IOException exc) {
				// just ignore
			}
	...
	public boolean requestWorkstationPrintsDocument(String workstation,
		...
		try {
			Node node = startNode;
			String message = "' passes packet on.\n";
			node.printRequestMessage(report, this, message);
			report.flush();
		} catch (IOException exc) {
			// just ignore
		}
	...
	public void writeAccountingReport(Writer report, String author, String title, String jobType) throws IOException {
