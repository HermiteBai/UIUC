...
animation filler::dfs::fillSolid( PNG & img, int x, int y, 
     ...
     */
	solidColorPicker pick = solidColorPicker(fillColor);
    return fill(img, x, y, pick, tolerance, frameFreq);
}
...
animation filler::dfs::fillGrid( PNG & img, int x, int y, 
     ...
     *  correct call to fill with the correct colorPicker parameter.
     */
	gridColorPicker pick = gridColorPicker(gridColor, gridSpacing);
    return fill(img, x, y, pick, tolerance, frameFreq);
}
...
animation filler::dfs::fillGradient( PNG & img, int x, int y, 
     ...
     *  correct call to fill with the correct colorPicker parameter.
     */
	gradientColorPicker pick = gradientColorPicker(fadeColor1, fadeColor2, radius, x, y);
    return fill(img, x, y, pick, tolerance, frameFreq);
}
...
animation filler::fill( PNG & img, int x, int y,
	...
	{
		done[i] = new bool[height];
	}

	//set all pixels to unprocessed
	for (int i = 0; i < width; i++)
	{
	...
	//initialise stacks for next pixels
	OrderingStructure<int> containerX;
	OrderingStructure<int> containerY;
	...
	while (!containerX.isEmpty() && !containerY.isEmpty())
	{
		currX = containerX.remove();
		currY = containerY.remove();

		if (currY < 0 || currX < 0 || currY >= height || currX >= width)
			continue;
		curr = img(currX, currY);
		distance = pow(curr->red - start.red, 2) + pow(curr->blue-start.blue, 2)+pow(curr->green - start.green,2);
		if (distance > tolerance)
			continue;
		if (curr != NULL && !done[currX][currY])
		{
			*curr = fillColor(currX, currY);

			if (currX + 1 < width)
			{
				containerX.add(currX + 1);
				containerY.add(currY);
			}

			if (currY + 1 < height)
			{
				containerX.add(currX);
				containerY.add(currY + 1);
			}
