...
class KDTree
{
    ...
    private:
		...
		void build(int start, int end, vector<Point<Dim> > & source, int dimension);
		Point<Dim> select(vector< Point<Dim> > & list, int start, int end, int mid, int dimension);

		int partition(vector< Point<Dim> > & list, int start, int end, int pivot, int dimension);

		void swap(Point<Dim> &a, Point<Dim> &b);
	
		Point<Dim> find(int start, int end, const Point<Dim> &query, int dimension) const;
