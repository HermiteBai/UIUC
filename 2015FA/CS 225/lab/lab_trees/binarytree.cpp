...
/**
 * Private helper function for mirroring the tree.
 * @param subRoot
 */
 ...
 * @param subRoot
 * @param vector
 * output values, and false otherwise. This is also the criterion for a binary tree     to be a binary search tree.
 */

template <typename T>
void BinaryTree<T>::isOrdered(const Node* subRoot, vector<T> &vec) const
{
	if (subRoot == NULL)
		return;
	isOrdered(subRoot->left, vec);
...
template <typename T>
void BinaryTree<T>::printPaths(const Node* subRoot, vector<T> &vec) const
{
	if (!subRoot)
		return;
	vec.push_back(subRoot->elem);
...
template <typename T>
void BinaryTree<T>::print(vector<T> vec) const
{
	vector<T> temp;
...
template <typename T>
void BinaryTree<T>::distanceHelper(const Node* subRoot, vector<T> &vec) const
{
	if (!subRoot)
		return;
	if (subRoot->left != NULL)
		distanceHelper(subRoot->left, vec);
	vec.push_back(height(root)-height(subRoot));
	if (subRoot->right != NULL)
		distanceHelper(subRoot->right, vec);
}
