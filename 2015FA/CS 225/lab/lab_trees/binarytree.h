...
class BinaryTree
{
    ...
    private:
		...
		/**
		 * Private helper function for mirroring the original tree.
		 * @param subRoot the current node in the recursion
		 */
		 ...
		 * @param vector
		 */
		void isOrdered(const Node* subRoot, vector<T> &vec) const;

		/**
		 * Private helper function to print paths
		 * @param subRoot
		 */
		void printPaths(const Node* subRoot, vector<T> &vec) const;

		void print(vector<T> vec) const;

		void distanceHelper(const Node* subRoot, vector<T> &vec) const;
};
