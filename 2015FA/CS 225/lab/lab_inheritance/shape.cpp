...
Shape::Shape()
{
	color_.red = DEFAULT_COLOR.red;
	color_.green = DEFAULT_COLOR.green;
	color_.blue = DEFAULT_COLOR.blue;

	center_.set_x(DEFAULT_CENTER.x());
	center_.set_y(DEFAULT_CENTER.y());
}

Shape::Shape(const Vector2& pcenter, const RGBAPixel& pcolor)
{
	color_.red = pcolor.red;
	color_.green = pcolor.green;
	color_.blue = pcolor.blue;

	center_.set_x(pcenter.x());
	center_.set_y(pcenter.y());
}
