...
void HuffmanTree::buildTree( const vector<Frequency> & frequencies ) {
	...
	{
		TreeNode *newNode = new TreeNode(freq);
		if (newNode != NULL)
			singleQueue.push(newNode);
	}

	if (singleQueue.size() + mergeQueue.size() <= 1)
		return;
	...
	while (singleQueue.size() + mergeQueue.size() > 1)
	{
		...
		minimum1 = removeSmallest(singleQueue, mergeQueue);
		minimum2 = removeSmallest(singleQueue, mergeQueue);

		TreeNode *internal = new TreeNode(minimum1->freq.getFrequency() + minimum2->freq.getFrequency());
		internal->left = minimum1;
		internal->right = minimum2;
