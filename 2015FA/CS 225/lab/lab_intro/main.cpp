    ...
    for (size_t yi = 0; yi < original.height(); yi++)
    {
        for (size_t xi = 0; xi < original.width(); xi++)
        {
			...
			if ((original(xi, yi)->red + amount) <= 255 &&
			    (original(xi, yi)->red + amount) >= 0)
				{
					original(xi, yi)->red += amount;
				}
			else if ((original(xi, yi)->red + amount) > 225)
					original(xi, yi)->red = 255;
			else
					original(xi, yi)->red = 0;

			if ((original(xi, yi)->blue + amount) <= 255 &&
				(original(xi, yi)->blue + amount) >= 0)
				{
					original(xi, yi)->blue += amount;	
				}
			else if ((original(xi, yi)->blue + amount) > 225)
			        original(xi, yi)->blue = 255;
            else
	                original(xi, yi)->blue = 0;

			if ((original(xi, yi)->green + amount) >= 0 &&
			    (original(xi, yi)->green + amount) <= 255)
				{
					original(xi, yi)->green += amount;
				}
			else if ((original(xi, yi)->green + amount) > 225)
...
PNG blendImages(PNG firstImage, PNG secondImage)
{
	...
	for (size_t x = 0; x < firstImage.width(); x++)
	{
		...
		{
			RGBAPixel color;
			color.red = (firstImage(x, y)->red + secondImage(x, y)->red)/2;
			color.green = (firstImage(x, y)->green + secondImage(x, y)->green)/2;
			color.blue = (firstImage(x, y)->blue + secondImage(x, y)->blue)/2;
			*firstImage(x, y) = color;
		}
	}
    return firstImage;
}
